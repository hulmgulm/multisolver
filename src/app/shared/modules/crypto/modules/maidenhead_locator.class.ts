...
export class MaidenheadLocator {
    ...
    public codes2Coords(inputString:string):string {
        let locators = inputString.toUpperCase().match(/[A-R]{2,2}([0-9]{2,2}[A-X]{2,2})+([0-9]{2,2})?/g);

        let returnData:Array<string> = [];

        if (locators) {
            for (let locator of locators) {
                let pairs:RegExpMatchArray = locator.match(/.{2,2}/g);
                ...
                if (pairs) {
                    ...
                    returnData.push(this.beautifyCoord([latitude, longitude]));
                }
            }
        }
