...
export class Gronsfeld {
    ...
    public decipher(inputString:string, password:string):string {
        ...
        password = password.toUpperCase().replace(/[^0-9]+/g, "");
		...
		if (password) {
            ...
            returnString = inputString.split("").map(function(c:string):string {
                ...
                if ( charCode >= 65 && charCode <= 90 ) {
					return String.fromCharCode(65 + (26 + (charCode - 65) - (password.charCodeAt(keyOffset++) - 0x30)) % 26);
				} else if (charCode >= 97 && charCode <= 122) {
					return String.fromCharCode(97 + (26 + (charCode - 97) - (password.charCodeAt(keyOffset++) - 0x30) ) % 26);
				}
