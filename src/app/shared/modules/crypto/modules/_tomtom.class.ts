...
import { TomTom } from './tomtom.class';
import { DataContainers } from '../data-containers/dataContainers.class';

export class _TomTom extends TomTom {

    private _Identifier:string = "TOMTOM";
    ...
    public do() {
        ...
        if ( this.checkConstraintsNormal() ) {
            ...
            let data:string = super.decode(DataContainers.getInstance().getData("Trimmed"));
    ...
    private checkConstraintsNormal():boolean {
       return DataContainers.getInstance().getData("NonAlphaNum").match(/[\\\\/]/) != null;
    }
    ...
    private checkConstraintsGuess():boolean {
       return DataContainers.getInstance().uniqueChars("Unchanged") >= 3;
    }
}